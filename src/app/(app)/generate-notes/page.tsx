...
type NoteStyle = 'comprehensive' | 'concise' | 'bullet-points' | 'outline';

export default function GenerateNotesPage() {
  const [documentContent, setDocumentContent] = React.useState('');
  const [noteStyle, setNoteStyle] = React.useState<NoteStyle>('comprehensive');
  const [focusArea, setFocusArea] = React.useState('');
  const [generatedNotes, setGeneratedNotes] = React.useState<string | null>(null);
  const [isLoading, setIsLoading] = React.useState(false);
  const [error, setError] = React.useState<string | null>(null);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Call the server-side API instead of the direct AI flow
      ...
      const errMsg = e instanceof Error ? e.message : 'An unexpected error occurred while generating notes.';
      setError(errMsg);
      console.error(e);
    } finally {
      setIsLoading(false);
    ...
    <div className="flex flex-col items-center py-8">
        ...
        <CardContent className="space-y-6">
          <div className="space-y-2">
            ...
            <Textarea
              ...
              onChange={(e) => {
                setDocumentContent(e.target.value);
                if (e.target.value.trim().length >= 50) setFormError(null);
              }}
              className={formError ? 'border-destructive' : ''}
            />
            {formError && <p className="text-sm text-destructive">{formError}</p>}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label htmlFor="noteStyle">Note Style</Label>
              <Select value={noteStyle} onValueChange={(value: NoteStyle) => setNoteStyle(value)}>
                <SelectTrigger>
                  <SelectValue placeholder="Select note style" />
                </SelectTrigger>
                <SelectContent>
              ...
              <p className="text-xs text-muted-foreground">Choose how detailed you want your notes to be.</p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setFocusArea(e.target.value)}
              />
              <p className="text-xs text-muted-foreground">Specify what to emphasize in the notes.</p>
            </div>
          </div>

          <Button
            onClick={handleGenerate}
            disabled={isLoading || documentContent.trim().length < 50}
      ...
      {generatedNotes && !error && (
        <Card className="w-full max-w-2xl mt-8 shadow-lg">
          ...
          <CardContent>
            <div className="p-4 bg-muted/50 rounded-md border min-h-[200px] max-h-[500px] overflow-auto whitespace-pre-wrap text-sm prose prose-sm max-w-none">
              {generatedNotes}
            </div>
          </CardContent>
